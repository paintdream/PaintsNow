#include "RenderFlowGraph.h"
#include "../../../../Utility/LeavesFlute/LeavesFlute.h"
#include "../../../../Core/System/StringStream.h"
#include "../../../../Utility/SnowyStream/Manager/RenderResourceManager.h"
...
RenderFlowGraph::Link* RenderFlowGraph::FindLink(ed::LinkId id) {
	for (auto& link : links)
		if (link.ID == id)
			return &link;
...
RenderFlowGraph::Pin* RenderFlowGraph::FindPin(ed::PinId id) {
	if (!id)
		return nullptr;
...
bool RenderFlowGraph::IsPinLinked(ed::PinId id) {
	if (!id)
		return false;
...
bool RenderFlowGraph::CanCreateLink(Pin* a, Pin* b) {
...
RenderFlowGraph::GraphNode* RenderFlowGraph::SpawnDoNNode() {
	...
	nodes.back().Inputs.emplace_back(GetNextId(), "Reset", PinType::Flow);
	nodes.back().Outputs.emplace_back(GetNextId(), "Exit", PinType::Flow);
	nodes.back().Outputs.emplace_back(GetNextId(), "Counter", PinType::Int);
...
RenderFlowGraph::GraphNode* RenderFlowGraph::SpawnOutputActionNode() {
	...
	nodes.back().Inputs.emplace_back(GetNextId(), "Event", PinType::Delegate);
...
RenderFlowGraph::GraphNode* RenderFlowGraph::SpawnSetTimerNode() {
	...
	nodes.back().Inputs.emplace_back(GetNextId(), "Function Name", PinType::Function);
	nodes.back().Inputs.emplace_back(GetNextId(), "Time", PinType::Float);
	nodes.back().Inputs.emplace_back(GetNextId(), "Looping", PinType::Bool);
	nodes.back().Outputs.emplace_back(GetNextId(), "", PinType::Flow);
...
RenderFlowGraph::GraphNode* RenderFlowGraph::SpawnTraceByChannelNode() {
	...
	nodes.back().Inputs.emplace_back(GetNextId(), "End", PinType::Int);
	nodes.back().Inputs.emplace_back(GetNextId(), "Trace Channel", PinType::Float);
	nodes.back().Inputs.emplace_back(GetNextId(), "Trace Complex", PinType::Bool);
	nodes.back().Inputs.emplace_back(GetNextId(), "Actors to Ignore", PinType::Int);
	nodes.back().Inputs.emplace_back(GetNextId(), "Draw Debug Type", PinType::Bool);
	nodes.back().Inputs.emplace_back(GetNextId(), "Ignore Self", PinType::Bool);
	nodes.back().Outputs.emplace_back(GetNextId(), "", PinType::Flow);
	nodes.back().Outputs.emplace_back(GetNextId(), "Out Hit", PinType::Float);
	nodes.back().Outputs.emplace_back(GetNextId(), "Return Value", PinType::Bool);
...
void RenderFlowGraph::BuildNodes() {
	for (auto& node : nodes)
		BuildNode(&node);
}

void RenderFlowGraph::InitializeGraph() {
	ed::Config config;

	config.SettingsFile = "Blueprints.json";
	config.UserPointer = this;
	config.LoadNodeSettings = [](ed::NodeId nodeId, char* data, void* userPointer) -> size_t {
		RenderFlowGraph* self = (RenderFlowGraph*)userPointer;
	...
	links.push_back(Link(GetNextLinkId(), nodes[5].Outputs[0].ID, nodes[7].Inputs[0].ID));
}
...
ImColor RenderFlowGraph::GetIconColor(PinType type) {
	switch (type) {
		default:
...
void RenderFlowGraph::DrawPinIcon(const Pin& pin, bool connected, int alpha) {
	IconType iconType;
	...
	ax::Widgets::Icon(ImVec2((float)pinIconSize, (float)pinIconSize), iconType, connected, color, ImColor(32, 32, 32, alpha));
};

void RenderFlowGraph::ShowStyleEditor(bool* show) {
...
void RenderFlowGraph::ShowLeftPane(float paneWidth) {
	auto& io = ImGui::GetIO();
	IRender& render = textureSaveIcon->GetRenderResourceManager().GetDevice();
	...
	int saveIconWidth = textureSaveIcon->description.dimension.x();
	int saveIconHeight = textureSaveIcon->description.dimension.y();
	int restoreIconWidth = textureRestoreIcon->description.dimension.x();
	int restoreIconHeight = textureRestoreIcon->description.dimension.y();
	...
	for (auto& node : nodes) {
		...
		if (node.SavedState.empty()) {
			...
			if (ImGui::IsItemActive())
				drawList->AddImage((ImTextureID)render.GetResourceDeviceHandle(textureSaveIcon->GetRenderResource()), ImGui::GetItemRectMin(), ImGui::GetItemRectMax(), ImVec2(0, 0), ImVec2(1, 1), IM_COL32(255, 255, 255, 96));
			else if (ImGui::IsItemHovered())
				drawList->AddImage((ImTextureID)render.GetResourceDeviceHandle(textureSaveIcon->GetRenderResource()), ImGui::GetItemRectMin(), ImGui::GetItemRectMax(), ImVec2(0, 0), ImVec2(1, 1), IM_COL32(255, 255, 255, 255));
			else
				drawList->AddImage((ImTextureID)render.GetResourceDeviceHandle(textureSaveIcon->GetRenderResource()), ImGui::GetItemRectMin(), ImGui::GetItemRectMax(), ImVec2(0, 0), ImVec2(1, 1), IM_COL32(255, 255, 255, 160));
		} else {
			ImGui::Dummy(ImVec2((float)saveIconWidth, (float)saveIconHeight));
			drawList->AddImage((ImTextureID)render.GetResourceDeviceHandle(textureSaveIcon->GetRenderResource()), ImGui::GetItemRectMin(), ImGui::GetItemRectMax(), ImVec2(0, 0), ImVec2(1, 1), IM_COL32(255, 255, 255, 32));
		}
		...
		if (!node.SavedState.empty()) {
			...
			if (ImGui::IsItemActive())
				drawList->AddImage((ImTextureID)render.GetResourceDeviceHandle(textureRestoreIcon->GetRenderResource()), ImGui::GetItemRectMin(), ImGui::GetItemRectMax(), ImVec2(0, 0), ImVec2(1, 1), IM_COL32(255, 255, 255, 96));
			else if (ImGui::IsItemHovered())
				drawList->AddImage((ImTextureID)render.GetResourceDeviceHandle(textureRestoreIcon->GetRenderResource()), ImGui::GetItemRectMin(), ImGui::GetItemRectMax(), ImVec2(0, 0), ImVec2(1, 1), IM_COL32(255, 255, 255, 255));
			else
				drawList->AddImage((ImTextureID)render.GetResourceDeviceHandle(textureRestoreIcon->GetRenderResource()), ImGui::GetItemRectMin(), ImGui::GetItemRectMax(), ImVec2(0, 0), ImVec2(1, 1), IM_COL32(255, 255, 255, 160));
		} else {
			ImGui::Dummy(ImVec2((float)restoreIconWidth, (float)restoreIconHeight));
			drawList->AddImage((ImTextureID)render.GetResourceDeviceHandle(textureRestoreIcon->GetRenderResource()), ImGui::GetItemRectMin(), ImGui::GetItemRectMax(), ImVec2(0, 0), ImVec2(1, 1), IM_COL32(255, 255, 255, 32));
		}
...
void RenderFlowGraph::ShowFrame() {
	IRender& render = textureHeaderBackground->GetRenderResourceManager().GetDevice();
	UpdateTouch();
	...
	ed::Begin("GraphNode editor");
	{
		...
		util::BlueprintNodeBuilder builder((ImTextureID)render.GetResourceDeviceHandle(textureHeaderBackground->GetRenderResource()), textureHeaderBackground->description.dimension.x(), textureHeaderBackground->description.dimension.y());
...
RenderFlowGraph::RenderFlowGraph() : pinIconSize(24), editor(nullptr) {
	InitializeGraph();
}
...
extern unsigned char BlueprintBackground_png[];
extern unsigned int BlueprintBackground_png_len;
extern unsigned char ic_restore_white_24dp_png[];
extern unsigned int ic_restore_white_24dp_png_len;
extern unsigned char ic_save_white_24dp_png[];
extern unsigned int ic_save_white_24dp_png_len;
...
void RenderFlowGraph::LeaveMainLoop() {
	textureHeaderBackground = nullptr;
	textureSaveIcon = nullptr;
	textureRestoreIcon = nullptr;
}

void RenderFlowGraph::TickRender(LeavesFlute& leavesFlute) {
	if (!textureHeaderBackground) {
		textureHeaderBackground = CreateTexture(leavesFlute.snowyStream, leavesFlute.GetInterfaces(), BlueprintBackground_png, BlueprintBackground_png_len);
		textureSaveIcon = CreateTexture(leavesFlute.snowyStream, leavesFlute.GetInterfaces(), ic_save_white_24dp_png, ic_save_white_24dp_png_len);
		textureRestoreIcon = CreateTexture(leavesFlute.snowyStream, leavesFlute.GetInterfaces(), ic_save_white_24dp_png, ic_save_white_24dp_png_len);
	} else {
		ShowFrame();
	}
}