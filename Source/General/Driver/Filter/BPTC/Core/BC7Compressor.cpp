...
#define struct_aligned(f) struct alignas(f)
	...
	struct_aligned(128) Mat {
		Mat() {
			const float data[kNumErrorMetrics][kNumColorChannels] = {
				{ 1.0f, 1.0f, 1.0f, 1.0f },
				{ sqrtf(0.3f), sqrtf(0.56f), sqrtf(0.11f), 1.0f }
			};

			memcpy(kErrorMetrics, data, sizeof(kErrorMetrics));
		}

		float kErrorMetrics[kNumErrorMetrics][kNumColorChannels];
	} matInst;

	const float *GetErrorMetric() { return matInst.kErrorMetrics[GetErrorMetricEnum()]; }
...
namespace BC7C
{
	...
	void CompressImageBC7(const BYTE* inBuf, BYTE* outBuf, int width, int height)
	{
		...
		BC7CompressionMode::MaxAnnealingIterations = min((int)BC7CompressionMode::kMaxAnnealingIterations, GetQualityLevel());
